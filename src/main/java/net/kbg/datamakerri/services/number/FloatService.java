...
import net.kb.datamaker.numbers.FloatFactory;
...
public class FloatService {
    ...
    public Optional<NumberValue> floatInRange(int lowEnd, int highEnd) {
        if (lowEnd >= highEnd) {
            log.error("lowEnd >= highEnd");
            return Optional.empty();
        }

        Float rslt = FloatFactory.floatInRange(lowEnd, highEnd);
        NumberValue numberValue = new NumberValue(rslt.floatValue(), "" + rslt.toString());
