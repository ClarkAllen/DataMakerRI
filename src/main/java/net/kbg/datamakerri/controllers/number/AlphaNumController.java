...
public class AlphaNumController {
    ...
    @Autowired
    AlphaNumService alphaNumService;

    @PutMapping("pattern")
    public ResponseEntity makePatternedString(@RequestBody PatternTemplate pattern) {
        ...
        if (rslt.isEmpty()) {
            ErrorMsg errorMsg = new ErrorMsg("400",
                    "Bad argument.  " +
                            ...
                            "and the symbols can not be a backslash " +
                            "and the pattern must contain at least one of the " +
                            "pattern symbols.");
